...
def plot_metric_trends(emotion_target: str, metric_type: str):
    ...
    for lr in learning_rates:
        file_path = pathlib.Path(f"emotion_trends/{emotion_target}/{lr:.2f}lr/{metric_type}.csv")
        if file_path.exists():
    ...
    fig = px.line(
        ...
        y=y_axis_label,
        color="k_fraction",
        labels={"k_fraction": "K Fraction", "learning_rate": "Learning Rate", y_axis_label: y_axis_label.capitalize().replace('_', ' ')},
        title=f"Learning Rate vs {y_axis_label.split('_')[1].capitalize()} for {emotion_target}"
    )
    
    # Save the figure to an HTML file
    save_path = pathlib.Path(f"emotion_trends/{emotion_target}/{metric_type}_lr_plot.html")
...
def categorize_scores(output_sentences):
	...
	total_counts = defaultdict(lambda: defaultdict(lambda: {f"{lower}-{upper}": 0 for lower, upper in categories}))
...
def save_average_scores_to_csv(output_sentences: list):
    ...
    average_output_ft = df_sentences[df_sentences['type'] == 'output-ft'].groupby('k_fraction')['score'].mean().reset_index()
    
    average_output_pe = df_sentences[df_sentences['type'] == 'output-pe']["score"].mean()
    
    average_output_ft['k_fraction'] = average_output_ft['k_fraction'].astype(int)
    pe_row = pd.DataFrame([{'k_fraction': 'output-pe', 'score': average_output_pe}])
...
def main():
    ...
    # )
    # savepath = thisdir.joinpath(f"output/{emotion_target}/distance_{lr:.2f}lr.html")
